...
/*
    ...
    */
    ...
    {
      metric: "theil",
      containerId: "chart-container-9",
      ledgerSelectId: "ledger-select-9",
      sliderId: "timeframe-slider-9",
      onChange: (data, config, minValue, maxValue) => {
        //console.log(`Range selected for Chart 1: ${minValue} to ${maxValue}`);
        // Update Chart 1 based on selected range
        updateChart(data, config, minValue, maxValue);
...
//    "tau=0.33",
  ...
  let fileName = "output/output-absolute_1000.csv";
  ...
  const chart_watermark = {
    ...
    afterDraw: (chart) => {
      ...
      const currentTheme = document.body.classList.contains('dark') ? 'darkThemeBtn' : 'lightThemeBtn';
      const watermarkSrc = currentTheme === 'darkThemeBtn' ? 'edi-white.png' : 'edi-black.png';
  ...
  $('#clustering_select').select2({
    ...
    $('#clustering_select').val(['explorers', 'staking']).trigger('change');
    

  $('#thresholding_select').select2({
    theme: "bootstrap-5",
    placeholder: "Thresholding",
    allowClear: false,
...
//  $( "#btnLoad" ).on( "click", function() {
  $( ".btnLoadFiles" ).bind( "click", function(e) {
    threshold = $('#thresholding_select').val();
    //clustering = $('#clustering_select').val();
    clustering = getSelectedValues();
    //console.log(`Threshold: ${threshold} - Clustering: ${clustering}`)
    if(threshold == "100") {
      if(clustering == "explorers") {
        fileName = "output-explorers-absolute_100.csv"
        console.log('Clustering: Explorers')
      } else if(clustering == "staking") {
        fileName = "output-staking_keys-absolute_100.csv"
        console.log('Clustering: Staking')
      } else if (clustering.join(',') == "explorers,staking") {
        fileName = "output-absolute_100.csv"
      } else {
        fileName = "output-no_clustering-absolute_100.csv"
      }
      
    } else if (threshold == "1000") {
      if(clustering == "explorers") {
        fileName = "output-explorers-absolute_1000.csv"
      } else if(clustering == "staking") {
        fileName = "output-staking_keys-absolute_1000.csv"
      } else if (clustering.join(',') == "explorers,staking") {
        fileName = "output-absolute_1000.csv"
      } else {
        fileName = "output-no_clustering-absolute_1000.csv"
      }
      
    } else if(threshold == "50p") {
      if(clustering == "explorers") {
        fileName = "output-explorers-percentage_0.5.csv"
      } else if(clustering == "staking") {
        fileName = "output-staking_keys-percentage_0.5.csv"
      } else if (clustering.join(',') == "explorers,staking") {
        fileName = "output-percentage_0.5.csv"
      } else {
        fileName = "output-no_clustering-percentage_0.5.csv"
      }
      
    } else if(threshold == "above") {
      if(clustering == "explorers") {
        fileName = "output-explorers-exclude_below_usd_cent.csv"
      } else if(clustering == "staking") {
        fileName = "output-staking_keys-exclude_below_usd_cent.csv"
      } else if (clustering.join(',') == "explorers,staking") {
        fileName = "output-exclude_below_usd_cent.csv"
      } else {
        fileName = "output-no_clustering-exclude_below_usd_cent.csv"
      }

    } else if(threshold == "none") {
      if(clustering == "explorers") {
        fileName = "output-explorers.csv"
      } else if(clustering == "staking") {
        fileName = "output-staking_keys.csv"
      } else if (clustering.join(',') == "explorers,staking") {
        fileName = "output.csv"
      } else {
        fileName = "output-no_clustering.csv"
      }
    } else {
      fileName = "output-absolute_1000.csv";
    }
