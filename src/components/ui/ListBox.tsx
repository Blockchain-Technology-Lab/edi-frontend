    ...
    <div className="form-control w-full max-w-xs h-full">
      ...
      <ul className="menu bg-base-300 rounded-box w-full shadow-md">
        {items.map((item) => {
          ...
          return (
            <li key={item.value} className="list-row w-full tracking-widest">
              <button
                type="button"
                onClick={() => onChange(item)}
                className={`flex justify-between items-center px-4 py-2 w-full text-left transition-all duration-150 rounded ${
                  isSelected ? 'bg-base-100 font-bold' : 'hover:bg-base-400'
                }`}
              >
