      ...
      <ul className=" menu bg-base-300 rounded-box w-full shadow-md">
        {items.map((item) => {
          ...
          return (
            <li
              key={item.value}
              className="list-row w-full m-1 tracking-widest font-stretch-expanded"
            >
              <button
                type="button"
                onClick={() => toggleItem(item)}
                className={`flex justify-between items-center px-1 py-1 w-full text-left ${
                  selected ? 'bg-base-100 font-bold' : 'hover:bg-base-400'
                }`}
              >
